...
void POMDPX::ReinitState(State& s,int pose) const
{
	POMDPXState& state = static_cast<POMDPXState&>(s);	

	double random_value = Random::RANDOM.NextDouble();
	POMDPXState *new_state=new POMDPXState(parser_->ComputeState(random_value));

	#ifndef SINGLE

	cout<<"New first :"<<new_state->vec_id[1]<<endl;
	cout<<"New second :"<<new_state->vec_id[2]<<endl;

	state.vec_id[1]=new_state->vec_id[1];
	state.vec_id[2]=new_state->vec_id[2];
