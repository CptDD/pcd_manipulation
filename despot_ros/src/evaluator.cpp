...
bool Evaluator::RunStep(int step, int round,string &ss) {
	...
	*out_ << "-----------------------------------Round " << round
	...
	if(boost::contains(action_temp,"d_")==1)
	{
		...
		cout<<"- Decomposed\t"<<e1<<"\t"<<l1<<"\t"<<e2<<"\t"<<l2<<endl;
		...
		//solver_->belief()->restart();
		model_->ReinitState(*state_,0);
	...
	}else
	{
		...
		boost::replace_all(state_temp,"- State:","");
		boost::replace_all(state_temp,"]","");
		cout<<state_temp<<endl;
		...
		string state_proc=state_tokens[0];
		boost::replace_all(state_proc,"robot_1:vp","");

		cout<<state_proc<<endl;
		...
		position_srv.request.current_position_label.data=state_proc;
		...
		if(classificationClient.call(classification_srv))
		{
			...
			cout<<"Classification resp :"<<classification_srv.response.first_type.data<<endl;
	...
	*out_<<endl;
	...
	if(boost::contains(action_temp,"d_")==0)
	{
		cout<<"- Decomposed\t"<<e1<<"\t"<<l1<<"\t"<<e2<<"\t"<<l2<<endl;
	}
...
void POMDPEvaluator::InitRound() {
	...
	bulb_scanner::position position_srv;
	position_srv.request.action.data=-1;
	position_srv.request.current_position_label.data="64";
