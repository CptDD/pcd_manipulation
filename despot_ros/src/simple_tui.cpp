...
string SimpleTUI::RunEvaluator(DSPOMDP *model, Evaluator *simulator,
  ...
  for (int round = start_run; round < start_run + num_runs; round++) {
    ...
    while(Globals::config.current_bulb_count<Globals::config.total_bulb_count)
    {
      /*
      ...
      bool terminal = simulator->RunStep(Globals::config.current_bulb_count,round,step_return);
