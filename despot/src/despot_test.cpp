...
int main(int argc, char* argv[]) {
  ...
  stringstream output_filename,pomdpx_filename;
  output_filename<<path<<"/results/results.txt";

  cout<<output_filename.str()<<endl;

  string pomdpx_path=ros::package::getPath("bulb_pomdpx_builder");
  pomdpx_filename<<pomdpx_path<<"/pomdpx/generated_pomdpx.pomdpx";

  string content=TUI().run(pomdpx_filename.str());

  //results_to_file(output_filename.str(),content);
  results_to_file(argv[1],content);
