...
OUTPUT=out_FHHOP.policy
GRAPH=graph_FHHOP.dot
OUTPUT_OP=simOUT_FHHOP.txt
PLOT=performance_FHHOP.png
...
### startsimulation
for j in 100 #1 10 100 500 750 1000 #1500
do
	for (( i=$STARTSIM; i<=$NRSIM; i++ ))
	do
		echo "============ At $i simulation for value $j ================="
		$SIMULATOR_PATH/pomdpOnlineSol_FHHOP $MODEL --budget 250 --simLen 30 --simNum $j --output $OUTPUT_OP --solverOP 2 --UnObsInitStateFile $INPUT >> FHHOPselectionTest
	done
...
FILEIN=simulationResults_FHHOP.txt
