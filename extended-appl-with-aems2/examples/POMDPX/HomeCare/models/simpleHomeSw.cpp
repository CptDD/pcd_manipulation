...
void stateTrFu(ofstream& pomdpxfile, int row, int col, int nrSW, int nrActions, int obstacle, int actionMap[][6], char *actionVec);
...
{ 
	...
	string actions = "N E S W F X", sw_states ="on  off", sw_observation = "O_on O_off unObs", reward = "reward_robot";
	string trueS = "true", falseS = "false", swObservability;

	const int obstacle = 7, switch1 = 2, nrActions = 6;
	int actionMap[2][6] = {	{-1, 0, 1, 0, 0, 0},
							{0, 1, 0, -1, 0, 0}}; // a = [row/column][N E S W f = flip Switch x= stay] order mathers
	char actionVec[] = "NESWFX";							
