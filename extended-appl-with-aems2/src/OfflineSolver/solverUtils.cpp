...
namespace momdp{
	...
	**********************************************************************/
	...
	SolverParams::SolverParams(void)
	{
		...
		initialUnObsState_file = "";
		initialUnObsState_val = -1;
	...
	// ***** OPMDP algorithm parameter	PAEL
		{"budget",	1, NULL, 'z'},		// budget to run the solver = max nodes expanded in beleif tree
		{ "solverOP",	1, NULL, 'Z' },	// option to use simulator/online control and real time visualision on/off
		{"UnObsInitStateFile", 1, NULL, 'K'},
		{NULL,0,0,0}
		...
		while (1) 
		{
			...
			case 'j': // bp improvement constant
				...
				break;
			case 'z': // budget for OPMDP algorithm
				p.OPbudget = atoi(optarg);
				break;
			case 'Z': // budget for OPMDP algorithm
				p.solverOption = atoi(optarg);
				break;
			case 'K': // initial POMDP state 
				p.initialUnObsState_file = string(optarg);
				break;
			case 'm': // memory limit
				{
