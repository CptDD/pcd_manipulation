...
#undef DEBUG_TRACE_ON
	...
	class LSEMAlphaPlaneTuple :public Tuple
	{
	public:
	...
	class DHS : public PointBasedAlgorithm
	{
	private:
	...
	public:
		...
		DHS(SharedPointer<MOMDP> problem, SolverParams * solverParams);
		virtual ~DHS(void);
		int algAEMS2, algLSEM;
		// DHS leaf selection based on deltaH_aems2 and deltaH_slem
		//DHSBeliefTreeNode* chooseLeaf(DHSBeliefTreeNode* root, double gamma);

		// new functions to generate the FRINGE childs of a node	PAEL
		void expandPrepare(DHSBeliefTreeNode* cn);
		DHSBeliefTreeNode* getOPNode(SharedPointer<BeliefWithState>& b_s);
		void expandOP(DHSBeliefTreeNode& cn);
		DHSBeliefTreeNode* initOPRoot(void);
		...
		void backupOPNode(DHSBeliefTreeNode* cn);
		void backupContributution(DHSBeliefTreeNode* cn, int maxAction);
		int backupUb_Lb(DHSBeliefTreeNode* cn);
		...
		DHSBeliefTreeNode* freeSubTree(DHSBeliefTreeNode *cn, int A, int O, int X, bool firstLevel);
		...
		void printTreeOP(DHSBeliefTreeNode *root, int depth, int currentDepth, REAL_VALUE Prob, int action, int observation)
		{
			if (depth >= root->depth )
			{
				//ofstream myfile;
				...
				{
					DHSBeliefTreeQEntry * Qa = &root->Q[Aval];
...
//					newMax->cacheIndex = node->cacheIndex.row;
		...
		*/
		...
		{
			LSEMAlphaPlaneTuple *attachedData = (LSEMAlphaPlaneTuple *)alpha->solverData;
