...
{
	//constructor
	void SampleBPOP::setup(SharedPointer<MOMDP> _problem, OPMDP* _solver )
	{
	...
	SampleBPOP::SampleBPOP ()
	{
	}

	void SampleBPOP::setRandomization(bool newFlag)
	{
	...
	list < cacherow_stval >SampleBPOP::sample (cacherow_stval currIndexRow, unsigned int currentRoot)
	{
		isRoot = false;
...
//		BeliefTreeNode & currentNode = *(solver->beliefCacheSet[currIndexRow.sval]->getRow(currIndexRow.row)->REACHABLE);
		...
		DEBUG_TRACE( cout << "SampleBPOP::sample ub" << ubVal << " lb " << lbVal << endl; );
//		DEBUG_TRACE( cout << "SampleBPOP::sample currentNode row: " << currentNode.cacheIndex.row << " sval " << currentNode.cacheIndex.sval << endl; );
		...
		DEBUG_TRACE( cout << "SampleBPOP::sample finalExcess " << finalExcess << endl; );
		...
		else
		{
			...
			DEBUG_TRACE( cout << "SampleBPOP::sample expectedError " << expectedError << endl; );
			DEBUG_TRACE( cout << "SampleBPOP::sample excessUncertainty " << excessUncertainty << endl; );
...
//				r.maxUBAction = solver->upperBoundSet->set[currIndexRow.sval]->dataTable->get(currIndexRow.row).UB_ACTION;
			...
			DEBUG_TRACE( cout << "SampleBPOP::sample r.maxUBAction " << r.maxUBAction << endl; );
...
//			cacherow_stval newIndexRow = currentNode.getNextState (i,j, Xn).cacheIndex;
			...
			DEBUG_TRACE( cout << "SampleBPOP::sample int i = r.maxUBAction; " << r.maxUBAction << endl; );
			DEBUG_TRACE( cout << "SampleBPOP::sample int Xn = r.maxExcessUncStateOutcome; " << Xn << endl; );
			DEBUG_TRACE( cout << "SampleBPOP::sample int j = r.maxExcessUncOutcome; " << j << endl; );
//			DEBUG_TRACE( cout << "SampleBPOP::sample newIndexRow row " << newIndexRow.row <<  " sval " << newIndexRow.sval << endl; );
...
cout << "End of one trial due to !( (currentNode.Q[i].ubVal == CB_QVAL_UNDEFINED || currentNode.Q[i].ubVal > curTargetUb))" << endl;
	...
	//    NA
	//Called by: sample()
	void SampleBPOP::getMaxExcessUncOutcome (BeliefTreeNode & cn, BPUpdateResult & r, unsigned int currentRoot) const
	{
	...
	void SampleBPOP::setBinManager (BinManagerSet * _bm)
	{
		bm = _bm;
	}

	bool SampleBPOP::CompareIfLowerBoundImprovesAction(int action, int observation, int xstate, BeliefTreeNode & currentNode,  unsigned int currentRoot)
	{
		int maxAction;
...
// doesnt seem to be used? - SYL 23072008
	double SampleBPOP::calculateLowerBoundValue(int action, BeliefTreeNode & currentNode)
	{
		double value = 0;
		outcome_prob_vector opv;
	...
	int SampleBPOP::chooseAction(BeliefTreeNode & currentNode)
	{
