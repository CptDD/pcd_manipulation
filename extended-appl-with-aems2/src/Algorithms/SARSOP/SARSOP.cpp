...
int SARSOP::getOptimalAction(SharedPointer<MOMDP> problem, int action, int observation, int Xn)
{
	return 0;
}
...
SARSOP::SARSOP(SharedPointer<MOMDP> problem, SolverParams * solverParams)
{
	...
	numBackups = 0;
	// clear  file to print out PAEL
	this->fileTreePrint.open("treeSARSOP.txt", ios::out | ios::trunc);
	...
	try
	{
		...
		while(!stop)
		{
			...
			{		
				...
				// print the "tree" like graph	PAEL
				
				//this->fileTreePrint.open("treeSARSOP.txt", ios::out | ios::app);
				//int depthLimit = 1;
				//cin >> depthLimit;
				//printTreeOP(this, beliefCacheSet[activeRoot]->getRow(0)->REACHABLE, depthLimit, 0, 1, -1, -1);
				//this->fileTreePrint << "=========" << endl << endl;
				//this->fileTreePrint.close();
...
cacherow_stval SARSOP::backup(list<cacherow_stval> beliefNStates)
{
	...
	LISTFOREACH(cacherow_stval, beliefNState,  beliefNStates) 
	{
		...
		//TODO: uncomment for good results
		nextRowNState = backup(rowNState);		// commented for testing initial bound	PAEL
	}//end FOR_EACH
