...
#define CHECK_INTERVAL 50
	...
	public:
		...
		virtual int getOptimalAction(SharedPointer<MOMDP> problem, int action, int observation, int Xn); // inserted for OPMDP	PAEL
		...
		// print out the graph (tree) till a well defined depth		PAEL
		

		void printTreeOP(SARSOP* solver, BeliefTreeNode *root, int depth, int currentDepth, REAL_VALUE Prob, int action, int observation)
		{
			if (depth >= currentDepth)
			{
				
				//myfile.open("treeARSOP.txt", ios::out | ios::app);
				fileTreePrint << action  << " "<< observation << " " << root->s->sval << " ";
				root->s->bvec->write(fileTreePrint) << " ";
				//myfile << root->LB << " " << root->UB << " ";
				fileTreePrint << std::setprecision(5)
					<< solver->beliefCacheSet[root->cacheIndex.sval]->getRow(root->cacheIndex.row)->LB << " " << 
					solver->beliefCacheSet[root->cacheIndex.sval]->getRow(root->cacheIndex.row)->UB << " ";
				fileTreePrint << currentDepth << " " << std::setprecision(5) << Prob << " " << root->cacheIndex.row <<endl;
				//upperBoundSet->set[root->s->sval]->cornerPoints.write(fileTreePrint) << endl;

				FOR(Aval, root->Q.size())
				{
					BeliefTreeQEntry  Qa = root->Q[Aval];

					FOR(Xni, Qa.getNumStateOutcomes())
					{
						BeliefTreeObsState * Xn = Qa.stateOutcomes[Xni];

						if (NULL != Xn)
						{
							FOR(Oval, Xn->getNumOutcomes())
							{
								BeliefTreeEdge *Xo = Xn->outcomes[Oval];
								if (NULL != Xo)
									printTreeOP(solver, Xo->nextState, depth, currentDepth + 1, Xo->obsProb, Aval, Oval);
								//else
									//myfile.close();
							}
						}
