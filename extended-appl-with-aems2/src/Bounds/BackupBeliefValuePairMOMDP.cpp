...
SharedPointer<BeliefValuePair> BackupBeliefValuePairMOMDP::backup(BeliefTreeNode* cn)
{
	...
	//uncommnet if out of testing bounds in OP		PAEL
...
double BackupBeliefValuePairMOMDP::getNewUBValueSimple(BeliefTreeNode& cn, int* maxUBActionP) 
{
	...
	if (maxVal< 0.4 && maxVal >0.39)
		cout << "the change!!! Test stuff"; // catching where and when does the UB change from initial value to smthg else	PAEL
