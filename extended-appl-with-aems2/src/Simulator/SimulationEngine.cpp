...
{ 
  ...
  void SimulationEngine::performActionUnobs(belief_vector& outBelUnobs, int action, const BeliefWithState& belSt, int currObsState) const
  {
    ...
    { 
        ...
        // by Elod Pall
        int currUnobsState;
        //TODO  assert needed if intitalUnObsState_val >= problem->initialBeliefStval->sval <= need to be checked if this val is the ref.
        //cout << "in " << this->solverParams->initialUnObsState_val << " > " << startBeliefVec->size() << endl;
        //cout << this->solverParams->initialUnObsState_val << " <" << startBeliefVec->size() << endl;
        assert(this->solverParams->initialUnObsState_val < startBeliefVec->size() && "The given initial switch state has more switches than in the given model");
        if (this->solverParams->initialUnObsState_val >= 0)
        {
        	currUnobsState = this->solverParams->initialUnObsState_val;
        	//cout << "file init state: " <<this->solverParams->initialUnObsState_filr << " cUS= " << currUnobsState <<endl;

        }
        else
        {
        	currUnobsState = chooseFromDistribution(*startBeliefVec);
        	//cout << "init file was not given";
        }
