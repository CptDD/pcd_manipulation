...
namespace momdp
{
     ...
     void Sample::expand(BeliefTreeNode& cn)
     {
	  ...
	  SharedPointer<SparseVector> pre(new SparseVector());
	  //SharedPointer<SparseVector> post(new SparseVector());//(new SparseVector());
			 ...
			 for(Observations::iterator oIter =  problem->observations->begin() ; oIter != problem->observations->end(); oIter ++)
			 {
			      ...
			      if (oprob > OBS_IS_ZERO_EPS) 
			      {
				   ...
				   e->obsProb = oprob * sprob; // P(o,Xn|cn.s,a) = P(Xn|cn.s,a) * P(o|Xn,cn.s,a)
					...
					//cout<<"Obs prob :"<<e->obsProb<<endl;
					
					
				   //e->nextState = getNode(problem->getNextBeliefStvalFast(sp, a, o, Xn, jspv));

					/*for(int i=0;i<sp->bvec->data.size();i++)
					{
						SparseVector_Entry ee=sp->bvec->data[i];
						cout<<"Index :"<<ee.index<<" Entry :"<<ee.value<<" ";
					}
					cout<<endl;*/
				   ...
				   /*Information i;
				   i.compute_information_gain(pre,post);*/
				 


				   /*cout<<"Sizer :"<<sp->bvec->size()<<" "<<sp->bvec->data.size()<<endl;
