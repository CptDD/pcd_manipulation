...
def get_actions():
	actions=['south','north','east','west','decision']
	return actions
...
def prepare_reward_entries(parent,bulbs):
	...
	if 'decision' in get_actions():
		tempEntry=ET.SubElement(parent,'Entry')
		tempInstance=ET.SubElement(tempEntry,'Instance').text='decision * * *'
		valueTable=ET.SubElement(tempEntry,'ValueTable')
		valueTable.text='20'
	...
	if 'decision' in get_actions():
		for k,v in points.items():
			for bulb in bulbs:
				for i in range(0,len(bulbs)):
					tempEntry=ET.SubElement(parent,'Entry')
					tempInstance=ET.SubElement(tempEntry,'Instance').text='decision vp'+str(k)+' '+bulb+' o'+bulbs[i]
					tempProbTable=ET.SubElement(tempEntry,'ProbTable')
					tempProbTable.text=str((1./len(bulbs)))
...
prepare_obs_function_entries(obsParameter1,points,bulbs,percs)
...
prepare_obs_function_entries(obsParameter2,points,bulbs,percs)
