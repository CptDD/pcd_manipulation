...
bool POMDPX::Step(State& s, double random_num, int action, double& reward,
	...
	//cout<<"S :"<<state<<"A :"<<action<<" O :"<<obs<<"R :"<<reward<<endl;
	return parser_->IsTerminalState(state.vec_id);
}

bool POMDPX::Step_IG(State& s,double random_num,int action,double& reward,OBS_TYPE& obs,Belief* b) const
{		
	...
	if(action!=4)
	{
		...
		double eig=i.compute_extended_expected_information_gain();
		//cout<<"Computed information gain is :"<<eig<<endl;
		reward=eig;
	}else
	{
		reward = parser_->GetReward(action); 
	}
