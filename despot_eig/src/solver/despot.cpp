...
void DESPOT::Expand(VNode* vnode,
	...
	//cout<<"Sizer :"<<vnode->particles().size()<<endl;
...
void DESPOT::Expand(QNode* qnode, ScenarioLowerBound* lb,
	...
	/**
	 *  Compute the expected information gain for all actions
	 */
		vector<double> eigs;
		Belief *pre;
		...
		{
			vector<State*>p=pre->Sample(500,particles,model);
			pre=new ParticleBelief(p,model);

		}else{
			pre=new ParticleBelief(particles,model);
		}
		
		for(int i=0;i<2;i++)
		{
			Belief *temp_pre=pre->MakeCopy();
			Information	ii(2);
			/*cout<<"Pre :"<<endl;
			cout<<pre->text()<<endl;*/
			ii.add_pre(temp_pre);
			temp_pre->Update(qnode->edge(),i);
			/*cout<<"Post :"<<endl;
			cout<<temp_pre->text()<<endl;*/
			ii.add_post(temp_pre);
			double eig=ii.compute_extended_expected_information_gain();
	...
	for (int i = 0; i < particles.size(); i++) {
		...
		/*bool terminal = model->Step_IG(*copy, streams.Entry(copy->scenario_id),
			qnode->edge(), reward, obs,eigs);*/

		

		//cout<<"Observation after step :"<<obs<<endl;
		//cout<<"Reward after step :"<<reward<<endl;
	...
	cout<<"Step reward is for action:"<<qnode->edge()<<" is "<<step_reward<<endl;
